    ...
    public virtual Task Over_Dev(bool async)
       => AssertQuery(
            ...
            ss => ss.Set<OrderDetail>().Select(od => new
            {
                ...
                MaxDiscount11 = EF.Functions.Over().PartitionBy(od.ProductID / 10, od.OrderID).OrderBy(od.OrderID).Rows(RowsPreceding.CurrentRow, 5).Max(od.Discount),

                //todo - this causing an error something to do with the <double> and default type mappings
                //avg1 = WindowFunctionsExtensions.Over().Average<double>(od.OrderID),
            }));
