...
{
    public class QuerySqlServerTest : QueryRelationalTestBase<NorthwindQuerySqlServerFixture>
    {
...
WHERE DATEPART(second, DATEADD(Second, 6E0, DATEADD(Minute, 1E0, DATEADD(Hour, 1E0, DATEADD(Day, 1E0, DATEADD(Year, 1, [o].[OrderDate])))))) = 6",
        ...
        /*public void DbFunction_StandardDiv()
        {
            using (var context = CreateContext())
            {
                var results = (from od in context.OrderDetails
                               select EF.Functions.StandardDeviation(od.UnitPrice)).First();

                Assert.Equal(29.82741788750095d, results);
            }

            Assert.Equal(
                @"SELECT TOP(1) Stdev([od].[UnitPrice])
FROM [Order Details] AS [od]",
                Sql);
        }*/
...
WHERE DATEPART(month, DATEADD(Month, 2, [o].[OrderDate])) = 5",
        ...
        public override void DbFunction_AddDays_Date()
        {
            ...
            Assert.Equal(
               @"SELECT [o].[OrderID], [o].[CustomerID], [o].[EmployeeID], [o].[OrderDate]
FROM [Orders] AS [o]
WHERE DATEPART(day, DATEADD(Day, 2, [o].[OrderDate])) = 5",
               Sql);
        }
        ...
        public override void DbFunction_AddHours_Date()
        {
            ...
            Assert.Equal(
              @"SELECT [o].[OrderID], [o].[CustomerID], [o].[EmployeeID], [o].[OrderDate]
FROM [Orders] AS [o]
WHERE DATEPART(hour, DATEADD(Hour, 11, [o].[OrderDate])) = 11",
              Sql);
        }
        ...
        public override void DbFunction_AddMinutes_Date()
        {
            ...
            Assert.Equal(
               @"SELECT [o].[OrderID], [o].[CustomerID], [o].[EmployeeID], [o].[OrderDate]
FROM [Orders] AS [o]
WHERE DATEPART(minute, DATEADD(Minute, 7, [o].[OrderDate])) = 7",
               Sql);
        }
        ...
        public override void DbFunction_AddSeconds_Date()
        {
            ...
            Assert.Equal(
               @"SELECT [o].[OrderID], [o].[CustomerID], [o].[EmployeeID], [o].[OrderDate]
FROM [Orders] AS [o]
WHERE DATEPART(second, DATEADD(Second, 6, [o].[OrderDate])) = 6",
               Sql);
        }

        public override void DbFunction_AddMilliSeconds_Date()
        {
            base.DbFunction_AddMilliSeconds_Date();

            Assert.Equal(
              @"SELECT [o].[OrderID], [o].[CustomerID], [o].[EmployeeID], [o].[OrderDate]
FROM [Orders] AS [o]
WHERE DATEPART(millisecond, DATEADD(Millisecond, 843, [o].[OrderDate])) = 843",
              Sql);
        }
...
WHERE CONVERT(date, [o].[OrderDate]) = CONVERT(date, @__Parse_0)",
        ...
        [ConditionalFact]
        public void DbFunction_PatIndex()
        {
            using (var context = CreateContext())
            {
                var results = (context.Customers.Where(c => EF.Functions.PatIndex("%ca%", c.CompanyName) > 0)).ToList();

                Assert.Equal(13, results.Count);
            }

            Assert.Equal(@"SELECT [c].[CustomerID], [c].[Address], [c].[City], [c].[CompanyName], [c].[ContactName], [c].[ContactTitle], [c].[Country], [c].[Fax], [c].[Phone], [c].[PostalCode], [c].[Region]
FROM [Customers] AS [c]
WHERE PatIndex(N'%ca%', [c].[CompanyName]) > 0",
            Sql);
        }

        [ConditionalFact]
        public void DbFunction_Like()
        {
            using (var context = CreateContext())
            {
                var results = (context.Customers.Where(c => EF.Functions.Like("%ca%", c.CompanyName))).ToList();
