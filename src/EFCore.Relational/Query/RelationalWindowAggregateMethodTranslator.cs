...
public class RelationalWindowAggregateMethodTranslator : IWindowAggregateMethodCallTranslator
{
    ...
    ///     doing so can result in application failures when updating to a new Entity Framework Core release.
    /// </summary>
    public virtual SqlFunctionExpression? Translate(MethodInfo method, IReadOnlyList<SqlExpression> arguments, IDiagnosticsLogger<DbLoggerCategory.Query> logger)
    {
    ...
    protected virtual CaseExpression[] BuildCaseExpression(IReadOnlyList<SqlExpression> arguments, SqlExpression? result = null)
